<div align="center">

[![English](https://img.shields.io/badge/lang-English-7c3aed?style=for-the-badge)](README.md)
[![中文](https://img.shields.io/badge/lang-中文-a78bfa?style=for-the-badge)](README-ZH.md)

<br>

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=28&duration=4000&pause=1000&color=7c3aed&center=true&vCenter=true&multiline=true&width=600&height=100&lines=Linmoqian;Biology+%2B+Tech+%3D+Infinite+Possibilities" alt="Typing SVG" />

[![Website](https://img.shields.io/badge/-linfengchen.com-7c3aed?style=for-the-badge&logo=data:image/svg+xml;base64,&logoSize=auto)](https://linfengchen.com)
[![GitHub](https://img.shields.io/badge/Linmoqian-333333?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Linmoqian)
[![Email](https://img.shields.io/badge-smkxxyclf@stu.scau.edu.cn-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:smkxxyclf@stu.scau.edu.cn)

</div>

<br>

## 📑 Quick Navigation

<div align="center">

[![About Me](https://img.shields.io/badge/👤_About-见个人简介-7c3aed?style=for-the-badge)](#-about-me)
[![Projects](https://img.shields.io/badge/🚀_Projects-精选项目-7c3aed?style=for-the-badge)](#-featured-projects)
[![Skills](https://img.shields.io/badge/🔬_Skills-热爱的领域-7c3aed?style=for-the-badge)](#-areas-of-interest)
[![Contact](https://img.shields.io/badge/📬_Contact-联系方式-7c3aed?style=for-the-badge)](#-contact)

</div>

<br>

<p align="center">
  <img src="https://raw.githubusercontent.com/Linmoqian/Linmoqian/assets/github-contribution-grid-snake.svg" alt="GitHub Contribution Snake Animation" loading="lazy" />
</p>

---

## ✨ About Me

![My Avatar](assets/images/avatar.jpg)

> **"Boundaries are not endings, but beginnings."**

I am a **Bioscience** student, but my soul acknowledges no boundaries.

At twenty, I stand at the intersection of two worlds—on one side, the miracle of cell division under the microscope; on the other, the rhythm of code flowing across the screen. True discoveries never happen in comfortable harbors, but in the tumultuous waves of unknown seas.

---

### 🧬 My Identity

| 🎓 | 🤖 | 🏸 |
|:---:|:---:|:---:|
| **B.S. Bioscience**<br>South China Agricultural University<br>2024-2028 | **AI Association President**<br>Head of Technology Dept.<br>Taurus Robotics Team | **Badminton Team Captain**<br>Media & Creative Dept.<br>Video Editing |

---

### 🌟 Philosophy

In my world, two rivers converge:

**🧬 DNA is Code** — Four bases writing life's most ancient program, each pair a carefully designed function call

**🤖 Code is Life** — Between 0 and 1 flows the pulse of a new era, every loop a rhythm of breath

**✍️ Pen is Heart's Extension** — Writing from the heart, driving the pen with intention. Text is not a tool, but the soul's breath, the projection of thought onto paper

**⚙️ Engineering is Thought Incarnate** — Engineering is the projection of theory into reality. With code and steel, we forge abstract ideas into tangible truth

When the microscope's eyepiece meets the display's glow, I see—

One river flowing for four billion years, from single cell to human, from chaos to consciousness;
One river flowing for half a century, yet reaching the stars, from computation to creation.

The true miracle happens in that boundary-blurring, collision-stirring sea. There, biology meets code, reason meets creation, known meets unknown.

---

### 💭 Reflections

> **"The starry heavens above and the moral law within, worth contemplating."**

Kant said, two things fill the mind with ever-increasing awe—
**The starry heavens above**: The vastness and order of the universe, the motion and laws of all things
**The moral law within**: The depth and radiance of humanity, the freedom and dignity of the soul

Between exploring life's mysteries and writing code, I often pause to gaze at the stars and examine my heart. Technology can reach the stars, but only reflection lets the soul take root.

---

## 🔬 Areas of Interest

**At the intersection of life and code, I found these directions:**

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch,opencv,react,nextjs,ros,stm32" alt="skills" />
</p>

| Domain | Interest |
|:---|:---:|
| 🔬 AI for Science | ██████████ 100% |
| 🧬 Bioinformatics | ██████████ 95% |
| 👁️ Computer Vision | ████████░ 85% |
| 🦾 Robotics Control | ████████░ 85% |
| ⚙️ Smart Hardware | ███████░ 80% |

---

## ⚙️ Engineering Practice

> **"Code is written for humans to read, only incidentally for machines to execute."**

I believe elegant code is the best documentation, and standardized processes are the cornerstone of team collaboration.

### 📐 Code Standards

| Language | Standard | Tools |
|:---|:---|:---|
| Python | PEP 8 | `black`, `ruff`, `mypy` |
| C++ | Google C++ Style | `clang-format`, `clang-tidy` |
| JavaScript/TypeScript | Airbnb Style Guide | `ESLint`, `Prettier` |
| React | React Official | `react-devtools` |

### 🧪 Testing Philosophy

```
Testing is not a burden, but a lighthouse of security in late nights.

├── Unit Testing  —— pytest/jest, coverage > 80%
├── Integration Test  —— API end-to-end verification
├── Performance Test  —— Stress testing, benchmarking
└── TDD Practice  —— Red, then Green, then Refactor
```

### 📚 Documentation

| Doc Type | Tools/Standards |
|:---|:---|
| API Docs | Sphinx / JSDoc |
| Deployment | Docker Compose / README |
| Comments | Google Style Docstrings |
| Changelog | CHANGELOG.md (Conventional Commits) |

### 🌳 Version Control

```
Git Flow Workflow
├── main      —— Production
├── develop   —— Development branch
├── feature/* —— Feature development
└── hotfix/*  —— Emergency fixes

Commit Convention: Conventional Commits (Chinese descriptions)
feat: New feature
fix: Bug fix
docs: Documentation
refactor: Refactoring
test: Testing
chore: Build/tools
```

### 🔍 Debugging & Optimization

- **Performance**: `py-spy` / `pprof` / Chrome DevTools
- **Memory Debug**: `memray` / `valgrind`
- **Logging**: Structured logging (JSON format)
- **Error Tracking**: Sentry / Self-built monitoring

---

## 📈 Open Source Contributions

<div align="center">

| 📊 | 🔀 | 🔳 | 👁️ | 🐛 |
|:---:|:---:|:---:|:---:|:---:|
| **1,200+**<br/>Total Contributions | **50+**<br/>PRs Merged | **15+**<br/>Active Repos | **100+**<br/>Code Reviews | **30+**<br/>Bug Fixes |

</div>

---

## 🏆 Honors & Awards

<div align="center">

| 🥇 | 🤖 | 🧮 | 🔬 | 🧬 |
|:---:|:---:|:---:|:---:|:---:|
| **iGEM**<br>Global Gold Medal | **RoboMaster**<br>National 1st Prize | **MCM/ICM**<br>M Prize | **AI for Science**<br>3rd Prize | **Life Sciences**<br>Ongoing |

</div>

---

## 🚀 Featured Projects

<div align="center">

[![Soybean Phenotype](https://img.shields.io/badge/🌾-Soybean_Phenotype-7c3aed?style=for-the-badge)](https://github.com/Linmoqian/soybean-phenotype)
[![Harvesting Robot](https://img.shields.io/badge/🤖-Harvesting_Robot-7c3aed?style=for-the-badge)](https://github.com/Linmoqian/harvesting-robot)
[![DNA Expression](https://img.shields.io/badge/🧬-DNA_Expression-7c3aed?style=for-the-badge)](https://github.com/Linmoqian/dna-expression)
[![AI Website](https://img.shields.io/badge/🌐-AI_Website-7c3aed?style=for-the-badge)](https://github.com/Linmoqian/ai-association-website)
[![LLM Robot Arm](https://img.shields.io/badge/🦾-LLM_Robot_Arm-7c3aed?style=for-the-badge)](https://github.com/Linmoqian/llm-robotic-arm)

</div>

<br>

<details>
<summary><b>🌾 Soybean Phenotype Recognition System</b></summary>
<br>

![Status](https://img.shields.io/badge/status-active-success?style=flat-square)
![Stars](https://img.shields.io/github/stars/Linmoqian/soybean-phenotype?style=flat-square)
![License](https://img.shields.io/github/license/Linmoqian/soybean-phenotype?style=flat-square)

When algorithms learn to read the language of fields, every plant has a name.

An agricultural vision recognition system based on improved YOLO, enabling machines to identify life forms among green waves. Between furrows and pixels, I translate nature's language with code.

| Metric | Value |
| :--- | :---: |
| 🎯 mAP@0.5 | 94.2% |
| ⚡ Inference Speed | 25 FPS |
| 📊 Dataset Size | 10,000+ labeled samples |

- **Tech Stack**: Python, PyTorch, YOLOv8, OpenCV
- **[Documentation](https://github.com/Linmoqian/soybean-phenotype/blob/main/docs/README.md)** · **[Source Code](https://github.com/Linmoqian/soybean-phenotype)**

</details>

<details>
<summary><b>🤖 Harvesting Robot</b></summary>
<br>

![Status](https://img.shields.io/badge/status-active-success?style=flat-square)
![Stars](https://img.shields.io/github/stars/Linmoqian/harvesting-robot?style=flat-square)
![License](https://img.shields.io/github/license/Linmoqian/harvesting-robot?style=flat-square)

Give the robotic arm eyes, let it learn gentle touch among fruits.

An intelligent harvesting robot based on ROS2, finding the perfect balance between precision and force—like human fingers, both steady and gentle.

| Metric | Value |
| :--- | :---: |
| 🎯 Recognition Accuracy | 96.5% |
| ⚡ Grasp Success Rate | 92.8% |
| 🔧 Degrees of Freedom | 6-DOF |

- **Tech Stack**: ROS2, Python, OpenCV, MoveIt, STM32
- **[Documentation](https://github.com/Linmoqian/harvesting-robot/blob/main/docs/README.md)** · **[Source Code](https://github.com/Linmoqian/harvesting-robot)**

</details>

<details>
<summary><b>🧬 DNA Expression Prediction Model</b></summary>
<br>

![Status](https://img.shields.io/badge/status-active-success?style=flat-square)
![Stars](https://img.shields.io/github/stars/Linmoqian/dna-expression?style=flat-square)
![License](https://img.shields.io/github/license/Linmoqian/dna-expression?style=flat-square)

Decode life's password with neural networks, seeing possibilities of fate in the arrangement of four bases.

A CNN-based DNA sequence expression prediction model, attempting to understand that book written three billion years ago—every ATGC is a poem waiting to be read.

| Metric | Value |
| :--- | :---: |
| 🎯 Pearson Correlation | 0.89 |
| 📊 Training Data | 50,000+ sequences |
| ⚡ Inference Time | < 10ms |

- **Tech Stack**: Python, TensorFlow, CNN, Biopython
- **[Documentation](https://github.com/Linmoqian/dna-expression/blob/main/docs/README.md)** · **[Source Code](https://github.com/Linmoqian/dna-expression)**

</details>

<details>
<summary><b>🌐 AI Association Website</b></summary>
<br>

![Status](https://img.shields.io/badge/status-active-success?style=flat-square)
![Stars](https://img.shields.io/github/stars/Linmoqian/ai-association-website?style=flat-square)
![License](https://img.shields.io/github/license/Linmoqian/ai-association-website?style=flat-square)

Make intelligent Q&A no longer high in the clouds, but within reach with warmth.

South China Agricultural University AI Association official website, building bridges with technology so every question finds an echo. Between code and kindness, I choose to give intelligence warmth.

| Metric | Value |
| :--- | :---: |
| ⚡ Page Load Speed | < 2s |
| 🎨 Lighthouse Score | 95+ |
| 📱 Responsive Design | Fully compatible |

- **Tech Stack**: Next.js, React, TypeScript, Tailwind CSS, Ollama
- **[Visit](https://scauaiu.xyz)** · **[Documentation](https://github.com/Linmoqian/ai-association-website/blob/main/docs/README.md)** · **[Source Code](https://github.com/Linmoqian/ai-association-website)**

</details>

<details>
<summary><b>🦾 LLM Robotic Arm Control</b></summary>
<br>

![Status](https://img.shields.io/badge/status-active-success?style=flat-square)
![Stars](https://img.shields.io/github/stars/Linmoqian/llm-robotic-arm?style=flat-square)
![License](https://img.shields.io/github/license/Linmoqian/llm-robotic-arm?style=flat-square)

When natural language meets mechanical motion, speech becomes action.

A natural language robotic arm control system based on LeRobot and Ollama, enabling machines to understand humanity's oldest instinct—language. Between words and actions, I build a bridge.

| Metric | Value |
| :--- | :---: |
| 🎯 Command Success Rate | 88.5% |
| ⚡ Response Time | < 3s |
| 🧠 Supported Models | Llama 3, Qwen 2.5 |

- **Tech Stack**: LeRobot, Ollama, Python, ROS2
- **[Documentation](https://github.com/Linmoqian/llm-robotic-arm/blob/main/docs/README.md)** · **[Source Code](https://github.com/Linmoqian/llm-robotic-arm)**

</details>

---

## 📊 GitHub Stats

<div align="center">

![Linmoqian's GitHub stats](https://github-readme-stats.vercel.app/api?username=Linmoqian&show_icons=true&theme=radical&hide_border=true&hide_title=true&include_all_commits=true&count_private=true)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Linmoqian&layout=compact&theme=radical&hide_border=true&hide_title=true&hide=markdown)

</div>

<div align="center">

![GitHub Streak](https://streak-stats.demolab.com?user=Linmoqian&theme=radical&hide_border=true&date_format=%5BY%5D%20n%2Fd%20%5BY%5D)

</div>

---

## 📬 Contact

<div align="center">

[![Gmail](https://img.shields.io/badge-smkxxyclf@stu.scau.edu.cn-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:smkxxyclf@stu.scau.edu.cn)
[![QQ Mail](https://img.shields.io/badge-3047076929@qq.com-12B7F5?style=flat-square&logo=tencentqq&logoColor=white)](mailto:3047076929@qq.com)
[![Website](https://img.shields.io/badge-linfengchen.com-7c3aed?style=flat-square&logo=data:image/svg+xml;base64,&logoSize=15)](https://linfengchen.com)

</div>

---

<div align="center">

---

> **"Where biology meets code,**
> **Where reason meets creation,**
> **Where known meets unknown.**
>
> **Not a battlefield of choice,**
> **But the home of the soul."**

---

⭐ **Star** · 🔔 **Follow** · 💬 **Connect**

<img src="https://raw.githubusercontent.com/Linmoqian/Linmoqian/output/github-contribution-grid-snake.svg" alt="GitHub Contribution Snake Animation - Footer" loading="lazy" style="max-width: 100%;" />

</div>
